...
export default function ProjectCard({
  ...
  const getCategoryColor = (key?: CategoryType) =>
    categories.find((c) => c.key === key)?.color || "bg-zinc-600";

  const CategoryBadge = ({ cat }: { cat?: CategoryType }) =>
    cat ? (
      <span
  ...
  // about과 list는 동일 UI 사용
  if (type === "about" || type === "list") {
